...
- **Content:** `data/speaking_patterns_v1.json` — 119 patterns across 33 families and 32 Speaking Intent categories (`it-was`: 21 patterns including a full 6-level expansion ladder on "It was difficult to ..." and a `to V` vs. `for + noun` contrast pair; 32 other families at 2-5 patterns each, spanning opinion, obligation, desire, reason, past habit, ongoing experience, guessing, uncertainty, decision-making, past attempts (`i-tried-to` vs. `i-tried-v-ing` is a second contrast pair), recall, agreement/disagreement, comparison, present-tense change, personalization, emotion/intuition, realization, process narration, scheduling, excitement, involuntary habits, status checks, examples, clarification requests, and discourse fillers). The data model (slots, examples, an optional `expansions` ladder, `relatedPatternIds`/`contrastPatternIds`, arbitrary `speakingIntent`/`familyId` grouping) supports the full 30-50 family / 150-250 pattern design target from the original spec without any code changes — adding a family is a seed-JSON change only, the same pattern as extending `data/chunk_drill_v1.json`. This pass has already reached the spec's 30-50 family target; further growth is now about depth (more examples/expansions per pattern) and the handful of remaining intent categories, not raw family count. 28 of the 119 patterns (every `core`-tagged flagship pattern across all 33 families) now carry a 4-level `expansions` ladder, not just "it-was-difficult-to"'s original 6-level one.
...
- **Practice flow:** all five spec drills are implemented. **Slot Replacement** (tap a candidate word/phrase into the pattern's blank, hear it spoken), **Korean Cue** (see the meaning, recall the pattern, reveal to check), **Random Slot** (every slot gets a random value the learner has to combine themselves before revealing -- unlike Slot Replacement, which drills one chosen slot and auto-fills the rest), and **Expansion** (step through a pattern's `expansions` ladder from its shortest form to its longest connected utterance) all reuse the same list-stepping store state (`stores/speakingPattern.ts`), sharing the double-tap guard / failed-save surfacing pattern already proven in `stores/chunkDrill.ts` -- Random Slot re-rolls its values on every `drillIndex` change, so drilling the same pattern for several rounds (as the Detail page's per-pattern entry does, 5 rounds) gives a different combination each time. **Free Speaking** is a separate, unscored practice screen (`SpeakingPatternFreeSpeakingView.vue`): a static Korean situational cue (`constants/freeSpeakingPrompts.ts`, 8 prompts) plus a handful of recommended patterns as scaffolding, no reveal or correctness check -- purely client-side content, no new backend model, since it needs no per-user progress tracking.
...
- Speaking Pattern Core has reached the spec's 30-50 family target (33 families, 119 patterns) — remaining growth is deepening existing families (more expansion ladders, more examples) and covering the last few uncommon intent categories, seed-content-only, no code changes needed
